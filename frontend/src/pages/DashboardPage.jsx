...
import {
  ...
  Loader2, ShieldCheck, LogOut, PlusCircle, Download, FileText
} from 'lucide-react'
...
export default function DashboardPage() {
  ...
  const handleVerComprobante = (insc) => {
    ...
    doc.text('Comprobante de Inscripcion (Vista Admin)', 20, 30)
    ...
    doc.setFontSize(9)
    doc.setFont('helvetica', 'bold')
    doc.text(insc.estado?.toUpperCase() || 'PENDIENTE', 167, 25, { align: 'center' })
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(14)
    doc.setFont('helvetica', 'bold')
    ...
    const rows = [
      ...
      ['Fecha Inscripcion', insc.fecha_inscripcion ? new Date(insc.fecha_inscripcion).toLocaleDateString('es-MX') : '-'],
      ['Metodo de Pago', insc.metodo_pago || '-'],
    ]
    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    ...
    doc.text(`ID Inscripcion: ${insc.id}`, 20, 291)
